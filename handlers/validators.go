	...
	if len(validators) > 0 {
		...
		for i, v := range validators {
			tableData[i] = []interface{}{
				...
				[]interface{}{
					fmt.Sprintf("%.4f %v", float64(v.CurrentBalance)/float64(utils.Config.Frontend.ClCurrencyDivisor)*price.GetStratisPrice(currency), currency),
					fmt.Sprintf("%.1f %v", float64(v.EffectiveBalance)/float64(utils.Config.Frontend.ClCurrencyDivisor)*price.GetStratisPrice(currency), currency),
				},
				v.State,
				[]interface{}{
					v.ActivationEpoch,
